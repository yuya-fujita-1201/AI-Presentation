...
## 第2周（2026-08-18）— 対象項目 ④ 人間らしい文章 ＋ ② 事実

- **文体パス**: notes 19枚を書き換え。「されています」11→2回、「としています」5→2回、①②の記号列挙0、全角ダッシュ0。主語のない受身文に発話主体を戻した
- **事実修正**（独立ファクトチェッカーの指摘 + 自分での原典確認）:
  - RefineBench: 動画由来の「自分の基準 vs 他AIの基準」という対比は論文（arXiv:2511.22173）に存在しない。原典は「自己修正 vs 外部からの指摘」の対比で、自己修正は5回で+1.8ポイント、外部指摘ありならほぼ満点。本文・notes・付録を訂正
  - 90.2%とトークン量の因果を「こちらの推論」と明記
  - 「実運用で広くマネージャー型」を2事例への言及に限定
  - フックの8回連続ブロックを「既定では」と明記
- **自分で外部検証した項目**: 90.2%／約15倍／分散80%、5パターン名、OpenAIの4基準と "Start with one agent whenever you can"、MASTの14モード・3カテゴリ・7フレームワーク・1,600件超・κ=0.88、Bun買収(2025-12)、7月18日のジョーク起源。いずれも一次資料と一致
- **機械ゲート**: gate_deck OK 45枚 / pptx 45枚・notes 0 ✅
- **コミット**: 41cc6d1
...
## 第3周（2026-08-18）— 対象項目 ⑥ 図解・視覚

- 状態と迂回路 / 増やす前の4条件 / サブエージェントの制約 に、ラベル付き専用SVGを新規作成（累計12枚）
- 箇条書きのみで下半分が空いていた S10・S24・S31 を image_text 化（bullets型は S39 の1枚のみに）
- 付録の出典表を「リンク付き資料名」セルに畳み、cell_size を 10-11 → 14-16 へ拡大
- 付録章扉のタイトル2行折り返しと罫線の接触を解消
- 「マネージャー」の多義使用（役割ラベル／構成パターン名）とキャプション重複を整理
- **機械ゲート**: gate_deck OK 45枚 / unzip エラーなし / pptx 45枚・notes 0 / preview 45枚すべて1280×720 / 参照アセット欠落0 ✅
- **コミット**: 510192e
- **注記**: 同一ブランチ上で別セッションが ai-eng-04 を並行改善中。ファイルは重複しないためコミットは意味単位で分離できている
...
全項目が目標8以上に到達。レイアウト不具合0件。④のみ8点で、指摘は「punch の対句型が12・16・17・41で反復」。

## 第4周（2026-08-18）— ④の仕上げ ＋ 付録の厚み

- **④**: 指摘された4箇所の対句を崩し、話者の判断・言い淀みを混ぜた。クロージングを5行→6行にし、失敗分類の回収を追加
- **枚数**: シリーズ実測（01=53 / 03=66 / 04=55）に対し05が45枚と最も薄かったため、付録を3枚追加して48枚に
  - 「もう1つの『グラフ』に気をつける」（実行の地図 vs 知識の地図。two_column）
  - 「線のほうにも、意味を持たせる」（エッジ＝契約、コンテキストパック）
  - 「この資料が、あえて言い切らなかったこと」（04の同型付録に倣った table）
- rubric の比較枚数を実測値へ修正
- **機械ゲート**: gate_deck OK 48枚 / unzip エラーなし / pptx 48枚・notes 0 / preview 48枚すべて1280×720 / 参照アセット欠落0 ✅
- **コミット**: f5a32e9

## 第5周（2026-08-18）— 最終採点の指摘を反映

最終採点（grader-final、全48枚を目視）で 58/70、④が7点、②で**実在するバグ**が発覚したため追加で1周。

- **② ページ参照バグ**: S39 の「付録の最後（45枚目）」が、付録3枚を追加したことで実体（S48）とずれていた。枚数を書かない「出典 3/3」というタイトル参照へ変更
- **④**: S9 の「という一点」を排除／S12・S17 の punch の対句を非対称化／S17・S19・S22 のキャプションの体言止め・「ではなく」型を解消／notes で「ではなく」が2回重なる2枚を整理（総数 23→21、04は17／55枚）
- **③**: S34 の検証ゲート4段階に業務シーンの具体例を追加
- 採点者が1度、ファイルを読み直さずに前回の内容を再送してきたため、**指摘4箇所を機械照合で実物確認**してから再採点を依頼した（graph-engineering の「Verifierの指摘も鵜呑みにしない」原則）
- **機械ゲート**: gate_deck OK 48枚 / unzip エラーなし / pptx 48枚・notes 0 / preview 48枚すべて1280×720 / 参照アセット欠落0 ✅
- **コミット**: 2c913f4

## ループ終了（2026-08-18）

**最終スコア 61/70。全7項目が目標の8点以上に到達したため終了。**

| 項目 | ベースライン | 第1周 | 第3周 | 最終 |
|---|---|---|---|---|
| ① 初心者への適合性 | 6 | 8 | 9 | 9 |
| ② 事実の正確性 | 8 | 9 | 9 | 9 |
| ③ 事例・比喩 | 7 | 8 | 9 | 9 |
...
※⑥が9→8に見えるのは採点者が別人のため（第3周は grader-r2、最終は grader-final）。どちらも目標の8以上。

- **ブランチ**: `loop/ge-deck-review`（baseline 8ced38a → 第5周 2c913f4 ＋ 記録コミット）
- **マージ**: 自動マージの3条件のうち「ユーザーの明示許可」が無いため実施しない。push・PR作成も未実施
- **注意**: 同一ブランチ上で別セッションが ai-eng-04 を並行改善している。マージ時はコミット単位で分けて確認すること

## 追記: ④専門の確定採点（grader-voice、2026-08-18）

文脈を持たない別エージェントに④だけを実数カウント付きで採点させたところ、**8/10**（grader-final と一致）。
- punch 28枚の文末は ます15／です7／ません4／疑問形1／体言止め1 に分散。04（27/31がですます）と同傾向
- notes の話者性（「だと思います」「正直」「んですね」「ちなみに」等）は18箇所。04では3箇所のみで、05のほうが濃い
- 「という一点」0／全角ダッシュ0／「の本質」1／「という設計」1

指摘された2件を追加で修正:
- **S14 と S22 の notes に類似度86%の重複文**（「ちなみに、複雑なグラフをやめて…揺り戻し」）。S22 側を損益分岐点の文脈（費用と待ち時間）に接続した独自の一文へ差し替え
- **S16・S17 の punch が隣接で「〜のほう」型**。S17 を「迂回路は、うまくいかなかったときの逃げ道です。ここを描き忘れると、失敗した瞬間に手順ごと止まります」へ
