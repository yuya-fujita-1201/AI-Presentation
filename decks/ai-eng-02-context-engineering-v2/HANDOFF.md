...
`decks/ai-eng-02-context-engineering-v2/`
...
旧フォルダ `decks/ai-eng-02-context-engineering/` と、過去のPR・worktreeは履歴用です。今後はv2を直接更新し、旧版へ逆同期しません。
...
1. `decks/ai-eng-02-context-engineering-v2/deck.json` または同フォルダの `assets/` を編集する
2. `/opt/homebrew/bin/python3 tools/build_deck.py decks/ai-eng-02-context-engineering-v2`
3. `/opt/homebrew/bin/python3 tools/preview_deck.py decks/ai-eng-02-context-engineering-v2`
4. 72枚のPNGを実際に開き、文字切れ・重なり・表の収まり・図とキャプションを確認する
5. `unzip -t`、python-pptx再パース、72枚一致、全スライド `has_notes_slide == False` を確認する
6. `bash pipeline/bin/gate_deck.sh decks/ai-eng-02-context-engineering-v2` を実行する
