...
| 5 | `diagram-knowledge-lifecycle.svg` | 02｜知識のライフサイクル | SVG | 討議→議事録→書記が投げる（AI が当日中に取り込む）→draftで更新→確認者がstableに→棚卸し の6ノードを輪に。棚卸しから draft への点線「変更が来たら「draft で更新」へ」 | 6（node-1..6） | 3「書記が投げる」と 5「確認者が stable に」（人手の2点を両方最濃に） |
...
- 事前レビュー後の変更: lifecycle の node-3 を「書記が投げる」に改題して最濃にし人手2点を対に（採点第1・2周の指摘）、practice-cycle の phase-2（育てる）を砂色に、overwrite の status 表記を「書き換え直後は draft、確認後に stable」に、「CR番号」を「変更管理の番号」に
...
本文・配置の正本は deck.json。既存6図の構図を継承し、構築日数の断定を試行での計測に訂正。形式検査の結果は検査ログに記録し、本文照合の verified と分けた。旧表は元版の制作履歴。修正版の実テキストはSVGとdeck.jsonを参照。
