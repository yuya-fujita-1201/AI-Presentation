...
def main() -> None:
    ...
    body += t(512, 714, "口を出す回数が減り、見守る時間が増えます", 24, ACCENT, 800)
    ...
    grid(
        ...
        "現場にすでにある観察源",
        [("テスト", "失敗→直す"), ("書き方の誤り", "指摘→修復"),
         ("レビュー", "コメント→再編集"), ("記録", "ログ→原因特定"),
         ("画面", "見た目→改善")],
    ...
    flow(
        ...
        "1ページの清書を回す最小ループ",
        [("目標", "採点表 8点"), ("改善", "1枚だけ修正"), ("機械の確認", "ビルド＋目視"),
         ("採点", "別担当が採点"), ("停止", "達成／3回／停滞")],
        highlight=4,
    ...
    flow(
        ...
        [("ReAct", "考える→行動"), ("Reflection", "失敗を記憶"),
         ...
         ("Loop Eng.", "停止・運用まで")],
        highlight=4,
        footer="既存の研究パターンを、実務の外周まで広げたものです",
    )
