...
- [../graph-engineering/subagent-design-in-practice.md](../graph-engineering/subagent-design-in-practice.md) — Subagents（焦点を絞ったサブタスクのために専門特化したエージェントを派生させる機能）・Permissions・Sessions・Hooks・MCPという提供機能の範囲と、Agent SDKがClaude Codeと同じツール群・エージェントループ・コンテキスト管理をPython/TypeScriptで提供するライブラリであることを、サブエージェント仕様のうち公式に裏付けられる範囲を示す根拠として使用（残りはauto字幕依存という確度差を明示するため）
- [../agent-capabilities/overview.md](../agent-capabilities/overview.md) — エージェントの定義の引用（自ら手順を計画しファイル読み取り・コマンド実行・コード編集を行う）と、Built-in tools / MCP / Skills・commands・memory / Plugins が同一の機能表に並置されていることを、4手段が排他でないことの根拠として使用
- [../agent-capabilities/what-is-mcp.md](../agent-capabilities/what-is-mcp.md) — MCPが「外部のツールやデータソースを接続する」機能として Built-in tools・Hooks・Subagents・Permissions・Sessions・Skills/commands/memory・Plugins と同じ表に並置されていることを、MCPがSkillの代替ではなく並列の口であることの根拠として使用
- [../agent-capabilities/choosing-skill-mcp-or-cli.md](../agent-capabilities/choosing-skill-mcp-or-cli.md) — Built-in tools にコマンド実行が含まれること（CLIを叩くのは標準機能であり事前コストがないこと）と、Built-in tools / MCP / Skills が並列に列挙され同時利用が前提であることの根拠
- [../agent-capabilities/prompts-and-project-rules.md](../agent-capabilities/prompts-and-project-rules.md) — 「Skills, commands, and memory」がプロジェクトの `.claude/` とホームディレクトリの `~/.claude/` から自動的に読み込まれることを、プロジェクトルールが「セッション開始時に自動で全文読まれるファイル」であることの公式側の根拠として使用
