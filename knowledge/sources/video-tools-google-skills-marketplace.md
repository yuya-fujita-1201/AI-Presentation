...
- [../agent-capabilities/writing-good-skills.md](../agent-capabilities/writing-good-skills.md) — firebase-basicsのUse when/Don't use whenの書き分けで誤発火を防ぐ例、本文30行の薄いスキルでも起動判定はdescriptionだけで決まる点、直近コミットでe.g./i.e.等の略記や曖昧表現を書き直す修正が入っている傾向、の3箇所で使用（いずれも聞き取り）
- [../agent-capabilities/choosing-skill-mcp-or-cli.md](../agent-capabilities/choosing-skill-mcp-or-cli.md) — gcloud CLI Skill for AI Agentsが他のgcloud関連スキルの前提になる「安全弁」であること、helpコマンドを毎回叩かせてleaf-levelで検証させる設計思想の2箇所で使用（いずれも聞き取り）
