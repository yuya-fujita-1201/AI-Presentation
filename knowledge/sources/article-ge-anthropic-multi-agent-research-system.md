...
- [../graph-engineering/failure-taxonomy-and-debugging.md](../graph-engineering/failure-taxonomy-and-debugging.md) — 実地の失敗事例（単純なクエリへの50サブエージェント生成、調査の重複、SEO最適化コンテンツの優先、十分な結果があっても探索を継続、ツール説明の不明瞭さと改善による40%の時間短縮）と、委譲時に目的・出力形式・ツール指定・境界を明示すべきという教訓を、MAST／AgentErrorTaxonomyの分類に対応づける実例として使用。Consoleでのシミュレーション駆動開発という教訓もデバッグ指針として使用
...
- [../graph-engineering/handoffs-and-ownership.md](../graph-engineering/handoffs-and-ownership.md) — 処理フロー（ユーザークエリ→リードエージェント→複数サブエージェント→結果統合→引用エージェントによる最終処理）が最終回答の所有権をリードに残すマネージャー型の実例にあたることの根拠、コンテキストウィンドウ超過に備えた計画のメモリ層保存という実装注意、および同期実行が情報フローのボトルネックを作るという課題と非同期化検討の根拠
- [../rag/rag-and-neighbors.md](../rag/rag-and-neighbors.md) — オーケストレーター・ワーカー構成による90.2%の性能改善と約15倍のトークン消費、「タスクの価値が支払いに見合う場合にのみ正当化される」という経済性条件。エージェント的検索を単発RAGと切り分け、一問一答には過剰だと述べる根拠
