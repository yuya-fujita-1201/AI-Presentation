...
- [../agent-capabilities/webmcp-and-frontier.md](../agent-capabilities/webmcp-and-frontier.md) — Host/Client/Server構造と Stdio / Streamable HTTP の2 transport を、WebMCP（ページ自身がツールを公開しブラウザ内で実行）と対比する軸として使用。また「MCP focuses solely on the protocol for context exchange—it does not dictate how AI applications use LLMs or manage the provided context.」を、規格が規定範囲を限定しているからこそ外側に別の公開経路を足す提案が成立しうる、という位置づけの根拠として使用
