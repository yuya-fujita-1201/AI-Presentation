...
- **ナレッジグラフとは別物**: 「グラフ」という一語は、実は複数の異なるものを指しうる。ソースによって3つとも4つとも数えられ、数え方自体が食い違っているが、いずれにせよ「実行の構造（誰が何をどう渡すか）」と「知識の構造（記憶・相関）」は別物であり、混同すると初心者は迷子になると両ソースとも警告している。個々の数え方の内訳と記憶としてのグラフの詳細は[knowledge-graph-as-memory.md](./knowledge-graph-as-memory.md)に一本化している
