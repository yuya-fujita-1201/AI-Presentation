...
**起動されるSkillの書き方とは、descriptionで確実に選ばれ、本文で狙った手順どおりに動かすための規則である。**
...
[google/skills解説動画](../sources/video-tools-google-skills-marketplace.md)が紹介するfirebase-basicsの例も、この一点を裏付ける実例になっている。Use whenとDon't use whenを明記し、使わない場面まで書くことで誤発火を防いでいるとし、自作スキルを書く際もこの粒度を真似るとよいと述べている（聞き取り）。**「何をするか」だけでなく「何をしないか」まで書く**のが、より発火精度を高める書き方である。

同動画は、本文が30行しかない薄いスキルでも起動判定はdescriptionだけで決まる点を指摘している（聞き取り）。**本文の分量とdescriptionの分量は無関係**であり、薄いスキルほど「短いから省略していい」ではなく、むしろdescriptionを削らずに書き切る必要がある。

同動画はまた、直近のコミット傾向として、e.g.やi.e.といった略記を普通の言葉に置き換える、曖昧な言い回しを動詞に直すといった、文章そのものを直す修正がまとまって入っていると紹介している（聞き取り）。**エージェントが読む文章としての解釈の割れを減らす**という観点は、Skillの本文だけでなくdescriptionの書き方にもそのまま適用できる。

## description以外のfrontmatter

[21個のSkillを運用した記事](../sources/article-tools-claude-code-skill-design.md)は、`name`・`description`以外にも実務で使うフロントマターフィールドを3つ挙げている。

- **`disable-model-invocation`**: デプロイなど副作用のあるSkillに設定し、モデルが勝手に起動しないようにする
- **`context: fork`**: サブエージェント実行で会話履歴を汚染しないようにする
- **`paths`**: Globパターンで自動発動条件を限定する

いずれも公式ドキュメント側の必須フィールドではないが、実務記事は自身の21個運用の中でこの3つを使い分けていると報告している。
...
ワークフローの書き方にも指針がある。同ドキュメントは、複雑なタスクにはチェックリスト形式のワークフローを、品質が重要なタスクには「Run validator → fix errors → repeat」というフィードバックループを組み込むことを推奨している。
...
[Anthropicのエンジニアリングブログ](../sources/article-tools-agent-skills-equipping-real-world.md)も、公式ドキュメントと並行してSkill開発の指針を4点挙げている。(1) 評価から始める（代表的なタスクで実行させ能力のギャップを特定する）、(2) スケールを見て設計する（SKILL.mdが大きくなったら内容を分離し相互参照する）、(3) Claudeの視点で考える（スキル名と説明の付け方に注意し、実際の使用パターンをモニタリングする）、(4) Claudeと反復する（Claude自身に成功したやり方やよくある失敗を再利用可能な形で記録させる）。評価を先に作る・書きすぎないという上記の方針と、細部の表現は違うが同じ方向を向いている。
...
規則を並べると難しく見えるが、始め方のハードルは低い。[21個運用した記事](../sources/article-tools-claude-code-skill-design.md)の著者は「最初はnameとdescriptionだけのシンプルなSKILL.mdで十分。使いながら育てていくのが一番のコツ」と述べ、Skillを持つことを「『自分専用のClaude Code』を育てることに近い」と総括している。作った後の見直し方（「引き算のメンテナンス」）は[distribution-and-governance.md](./distribution-and-governance.md)で扱う。
