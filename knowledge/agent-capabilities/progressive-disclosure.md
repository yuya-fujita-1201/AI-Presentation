...
公式が3階層、動画が2段階と数え方は違うが、指しているものは同じである。動画側は Level 3（参照ファイル）を独立した段階として数えていないだけで、「メタデータだけ常時 → 必要時に本文」という骨格は一致している。数え方の違いより重要なのは、**「常に読まれる部分」と「条件付きで読まれる部分」が分かれている**という構造そのものである。
...
なお同動画は、progressive disclosureの仕組みにより通常はスキルのdescription部分だけが読み込まれ、必要になった時に本文が展開されると説明しており（[04:00]、聞き取り）、この点は前掲の公式ドキュメントの記述と独立に一致する。

同動画はまた、SKILL.mdの中身は9割以上がMarkdownの散文であり、Pythonは一部のスキルに付随するスクリプト部分のみだとしている（[01:00]、聞き取り）。段階的開示が主に軽くしているのは実行コードの分量ではなく、説明文の読み込み量である。

## なぜこの設計が要るのか——コンテキストは有限で、しかも共有財である

段階的開示は「あれば嬉しい最適化」ではなく、制約から要請された設計である。
...
圧迫されると何が起きるのか。[CLAUDE.md運用の実務記事](../sources/article-he-claude-md-best-practices.md)はこれを「コンテキスト汚染」と呼び、詰め込むほど守られない指示が増えると指摘している（ワーキングメモリの比喩と具体的な行数の目安は[プロンプトとプロジェクトルール](./prompts-and-project-rules.md)で扱う）。
...
段階的開示はSkillの機能名ではなく、**設計のパターン**である。同じ考え方はプロジェクトルール（`CLAUDE.md`）の書き方にも適用でき、[CLAUDE.md運用の実務記事](../sources/article-he-claude-md-best-practices.md)も詳細情報を別ファイルに分け本体には場所だけ案内する設計を推奨している。手段が違っても**常時読ませる部分は小さく保ち、深い情報は場所だけ案内する**という指針は同じである。具体的な分割例・引用・行数目安は[prompts-and-project-rules.md](./prompts-and-project-rules.md)にまとめている。
