...
公式クイックスタート自身も「このチュートリアルは主にToolsに焦点を当てる」としており、本バンドルもToolsを中心に扱う。**Tools＝AIに使わせる関数を外から生やす仕組み**が、以降の説明の中心になる。
...
選び方は単純で、**ローカルのツールならStdio、外部サービスならStreamable HTTP**である。手元で `npx` や `uv` を叩く形で起動するMCPサーバーは前者である。
